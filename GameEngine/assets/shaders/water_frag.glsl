...
uniform sampler2D textSamp03;	// Specular map
...
void main()  
{
	...
	vec3 tex0_RGB = texture( textSamp00, fUVx2.st + fiTime / 20.f ).rgb;
	vec3 tex1_RGB = texture( textSamp00, vec2(fUVx2.t + (-fiTime / 40.f), fUVx2.s)).rgb;
	vec3 tex2_RGB = texture( textSamp02, fUVx2.st + fiTime / 20.f ).rgb;
	vec3 tex2i_RGB = texture( textSamp02, vec2(fUVx2.t + (-fiTime / 40.f), fUVx2.s) ).rgb;
	vec3 tex3_RGB = texture( textSamp03, fUVx2.st + fiTime / 20.f ).rgb;
	...
	tangent_normal = (tex2i_RGB * 2.0) - 1.0;
	bitangent_normal = cross(tangent_normal, fNormal.xyz);

	worldSpaceNormal = mix(
	worldSpaceNormal,
	vec3(tangent_normal.r * tangent_normal.xyz + tangent_normal.b * fNormal.xyz + tangent_normal.g * bitangent_normal.xyz),
	0.5
	);
