...
bool cVAOManager::LoadInstanceIntoVAO(std::string fileName, cMesh& theMesh, sModelDrawInfo& drawInfo, const std::vector<glm::mat4>& offsets, unsigned int shaderProgramID)
{
	// Write some code to copy the infomation from cMesh& theMesh
	//  to the sModelDrawInfo& drawInfo...
...
//	glGenBuffers(1, &vertex_buffer);
	...
	BIND OFFSET TRANSFORMS;

	*/
	const mat4* data = offsets.data();
	GLuint offsetBuffer;
	GLint offsetLocation = glGetAttribLocation(shaderProgramID, "vMatModel");
	glGenBuffers(1, &offsetBuffer);
	glBindBuffer(GL_ARRAY_BUFFER, offsetBuffer);
	glBufferData(GL_ARRAY_BUFFER, sizeof(data), data, GL_STATIC_DRAW);

	glEnableVertexAttribArray(offsetLocation);
	glEnableVertexAttribArray(offsetLocation + 1);
	glEnableVertexAttribArray(offsetLocation + 2);
	glEnableVertexAttribArray(offsetLocation + 3);

	glVertexAttribPointer(offsetLocation, 4, GL_FLOAT, GL_FALSE, sizeof(mat4), (void*)(sizeof(float) * 0));
	glVertexAttribPointer(offsetLocation + 1, 4, GL_FLOAT, GL_FALSE, sizeof(mat4), (void*)(sizeof(float) * 4));
	glVertexAttribPointer(offsetLocation + 2, 4, GL_FLOAT, GL_FALSE, sizeof(mat4), (void*)(sizeof(float) * 8));
	glVertexAttribPointer(offsetLocation + 3, 4, GL_FLOAT, GL_FALSE, sizeof(mat4), (void*)(sizeof(float) * 12));

	glVertexAttribDivisor(offsetLocation, 1);
	glVertexAttribDivisor(offsetLocation + 1, 1);
	glVertexAttribDivisor(offsetLocation + 2, 1);
	glVertexAttribDivisor(offsetLocation + 3, 1);
